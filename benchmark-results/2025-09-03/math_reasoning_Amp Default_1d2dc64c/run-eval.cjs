...
const evalSpec = JSON.parse(readFileSync('benchmark-results/2025-09-03/math_reasoning_Amp Default_1d2dc64c/eval-spec.json', 'utf-8'))
const results = {
  repo: evalSpec.repo,
  rev: evalSpec.rev,
  questions: []
...
for (let i = 0; i < evalSpec.questions.length; i++) {
  ...
  try {
    ...
    actualOutput = execFileSync('node', fullCommand, {
      encoding: 'utf-8',
      timeout: 60000,
...
writeFileSync('benchmark-results/2025-09-03/math_reasoning_Amp Default_1d2dc64c/result.1756927495850.json', JSON.stringify(results, null, 2))
console.log('Results written to benchmark-results/2025-09-03/math_reasoning_Amp Default_1d2dc64c/result.1756927495850.json')
